...
- The source supported agent is one of Claude Code, Codex CLI, OpenCode CLI, Gemini CLI, or Cursor.
...
- `all` expands only to Claude Code, Codex CLI, OpenCode CLI, Gemini CLI, and Cursor.
...
- Gemini CLI source: one JSON server entry keyed by name.
...
- Gemini CLI output: one JSON server entry keyed by name.
...
- Output snippets are not project config files such as `.mcp.json`, `.codex/config.toml`, `opencode.json`, `.gemini/settings.json`, or `.cursor/mcp.json`.
